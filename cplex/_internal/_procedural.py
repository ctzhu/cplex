            ...
            if self._pyenv._callback_exception is not None:
                ...
                if isinstance(callback_exception, ErrorChannelMessage):
                    ...
                    try:
                        ...
                        # We get here, for example, if the user hits Ctrl+C
                        # during a solve.  In that case, the solve should
                        # exit with a status of 'aborted'.
...
def newcols(env, lp, obj, lb, ub, xctype, colname, enc=default_encoding):
    ...
    with LAU.double_c_array(obj) as c_obj:
        with LAU.double_c_array(lb) as c_lb:
          with LAU.double_c_array(ub) as c_ub:
...
def delcols(env, lp, begin, end):
    ...
    _delbyrange(env, lp, begin, end, delfn)
...
def newrows(env, lp, rhs, sense, rngval, rowname, enc=default_encoding):
    ...
    with LAU.double_c_array(rhs) as c_rhs:
        with LAU.double_c_array(rngval) as c_rng:
...
def chgrngval(env, lp, indices, values):
    with LAU.int_c_array(indices) as c_ind:
        with LAU.double_c_array(values) as c_val:
            status = CR.CPXXchgrngval(env, lp, len(indices),
                                      c_ind, c_val)
    check_status(env, status)
...
def _delbyrange(env, lp, begin, end, delfn):
    status = delfn(env, lp, begin, end)
    check_status(env, status)

# FIXME: There is an inconsistency between API's where with some we have
#        delete by index and delete set, and with others we have only
#        delete range ... should we try to make that consistent?
def _delbyindex(env, lp, idx, delfn):
    status = delfn(env, lp, idx)
    check_status(env, status)
...
def dellonganno(env, lp, idx):
    delfn = CR.CPXXdellongannotation
    _delbyindex(env, lp, idx, delfn)

def deldblanno(env, lp, idx):
    delfn = CR.CPXXdeldblannotation
    _delbyindex(env, lp, idx, delfn)
...
def addpwl(env, lp, vary, varx, preslope, postslope, nbreaks,
    ...
    status = CR.CPXXaddpwl(env, lp, vary, varx, preslope, postslope,
                           nbreaks,
                           LAU.double_list_to_array(breakx),
                           LAU.double_list_to_array(breaky),
                           cpx_decode_noop3(name, enc))
    check_status(env, status)
...
def getbase(env, lp):
    ...
    status = CR.CPXXgetbase(env, lp, cstat, rstat)
    check_status(env, status)
    return (LAU.array_to_list(cstat, numcols), LAU.array_to_list(rstat, numrows))
...
def getmipstarts_effort(env, lp, begin, end):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    if surplus.value() == 0:
        return ([0] * _rangelen(begin, end), [], [], [0] * _rangelen(begin, end))
